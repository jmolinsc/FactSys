            ...
            <div class="card">
                <div class="card-header">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <span id="card_title">
                            <a href="{{ route('fabricantes.create') }}" class="btn btn-primary btn-md float-right"
                                data-placement="left">
...
@section('js')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            new DataTable('#tblManufacturers', {
                responsive: true,
                fixedHeader: true,
                ajax: {
                    url: '{{ route('manufacturers.list') }}',
                    dataSrc: 'data'
                },
                columns: [{
                        data: 'id'
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                          ...
                          /*   return '<a class="btn btn-sm btn-primary" href="/fabricantes/' +
                                row.id + '/edit">Editar</a>' +
                                '<button class="btn btn-sm btn-danger" onclick="deleteFamily(' +
                                row.id + ')">Eliminar</button>';
 */
                                return '<a class="btn btn-info btn-sm" href="/fabricantes/' +
                                row.id + '/edit"><i class="fas fa-pencil-alt"></i></a>' +
                                '<a class="btn btn-danger btn-sm" onclick="deleteClient(' +
        ...
        function deleteFamily(familiaId) {
            Swal.fire({
                title: "Eliminar",
                ...
                if (result.isConfirmed) {
                    fetch('/fabricantes/' + familiaId, {
                        ...
                        .then(data => {
                            ...
                            $('#tblManufacturers').DataTable().ajax.reload();
                        })
                        .catch(error => {
                            // Manejar errores
